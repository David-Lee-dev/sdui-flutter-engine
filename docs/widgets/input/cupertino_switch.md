...
Builds a controlled iOS-style [CupertinoSwitch] using the engine's truthiness rules. This primitive mirrors the corresponding Flutter widget described by its implementation.
...
| `scale` | `number` | — | `1.0` | Yes | uniform size factor; scales both the switch and the layout footprint (1.0 = the native iOS size). |
| `semantics_label` | `text` | — | `null` | Yes | accessibility label. |
| `active_track_color` | `color` | — | `null` | Yes | track color while on. |
| `inactive_track_color` | `color` | — | `null` | Yes | track color while off. |
| `thumb_color` | `color` | — | `null` | Yes | thumb color. |
| `inactive_thumb_color` | `color` | — | `null` | Yes | thumb color while off. |
