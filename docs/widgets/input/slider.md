...
Builds a controlled Flutter [Slider] from the bound engine value. This primitive mirrors the corresponding Flutter widget described by its implementation.
...
| `min` | `number` | — | `0.0` | Yes | lower bound of the slider value. |
| `max` | `number` | — | `1.0` | Yes | upper bound of the slider value. |
| `divisions` | `integer` | — | `null` | Yes | splits the slider range into discrete intervals. |
| `active_color` | `color` | — | `null` | Yes | color used for the selected or active state. |
| `inactive_color` | `color` | — | `null` | Yes | color used for the inactive slider track. |
| `thumb_color` | `color` | — | `null` | Yes | color of the scrollbar thumb. |
| `label` | `text` | — | `null` | Yes | text displayed for the current slider value. |
