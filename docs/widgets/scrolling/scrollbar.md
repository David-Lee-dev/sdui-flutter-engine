...
Builds Flutter's [Scrollbar] around the node's first child. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `radius` | `size` (scaled by EngineMetrics) | — | `null` | Yes | corner radius of indicator dots. |
| `thumb_visibility` | `flag` | — | `null` | Yes | keeps the scrollbar thumb visible when true. |
| `track_visibility` | `flag` | — | `null` | Yes | keeps the scrollbar track visible when true. |
| `interactive` | `flag` | — | `null` | Yes | allows pointer interaction with the scrollbar. |
| `thickness` | `size` (scaled by EngineMetrics) | — | `null` | Yes | width of the scrollbar thumb. |
| `thumb_color` | `color` | — | `null` | Yes | color of the scrollbar thumb. |
| `track_color` | `color` | — | `null` | Yes | color of the scrollbar track. |
