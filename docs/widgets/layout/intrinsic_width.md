...
Builds Flutter's [IntrinsicWidth] for an `intrinsicWidth` node. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `step_width` | `size` (scaled by EngineMetrics) | — | `null` | Yes | rounds intrinsic width up to a multiple of this value. |
| `step_height` | `size` (scaled by EngineMetrics) | — | `null` | Yes | rounds intrinsic height up to a multiple of this value. |
