...
Builds Flutter's [Positioned], supplying an empty child when required. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `left` | `offset` | — | `null` | Yes | offset from the parent left edge. |
| `top` | `offset` | — | `null` | Yes | offset from the parent top edge. |
| `right` | `offset` | — | `null` | Yes | offset from the parent right edge. |
| `bottom` | `offset` | — | `null` | Yes | offset from the parent bottom edge. |
