...
Builds [Positioned.fill] for a `positionedFill` node. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `left` | `offset` | — | `0.0` | Yes | offset from the parent left edge. |
| `top` | `offset` | — | `0.0` | Yes | offset from the parent top edge. |
| `right` | `offset` | — | `0.0` | Yes | offset from the parent right edge. |
| `bottom` | `offset` | — | `0.0` | Yes | offset from the parent bottom edge. |
