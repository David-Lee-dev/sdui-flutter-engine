...
Builds Flutter's [FractionallySizedBox] for a `fractionallySizedBox` node. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `width_factor` | `number` | — | `null` | Yes | sizes width to this multiple of the child width. |
| `height_factor` | `number` | — | `null` | Yes | sizes height to this multiple of the child height. |
