...
Removes parent constraints from selected child axes. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `alignment` | `alignment` | top_left, top_center, top_right, center_left, center, center_right, bottom_left, bottom_center, bottom_right | `center` | Yes | child alignment. |
| `constrained_axis` | `axis` | horizontal, vertical | `null` | Yes | axis whose constraints remain. |
| `clip_behavior` | `clip` | none, hard_edge, anti_alias, anti_alias_with_save_layer | `none` | Yes | overflow clipping behavior. |
