...
Builds Flutter's [OverflowBox] for an `overflowBox` node. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `min_width` | `size` (scaled by EngineMetrics) | — | `null` | Yes | minimum permitted width. |
| `max_width` | `size` (scaled by EngineMetrics) | — | `null` | Yes | maximum permitted width. |
| `min_height` | `size` (scaled by EngineMetrics) | — | `null` | Yes | minimum permitted height. |
| `max_height` | `size` (scaled by EngineMetrics) | — | `null` | Yes | maximum permitted height. |
