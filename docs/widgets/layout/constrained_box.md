...
Builds Flutter's [ConstrainedBox] with unconstrained defaults for invalid props. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `constraints` | `constraints` | — | `unconstrained` | Yes | minimum and maximum dimensions imposed on the child. |
