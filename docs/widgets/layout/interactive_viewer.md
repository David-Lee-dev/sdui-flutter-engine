...
Lets users pan and scale a child. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `min_scale` | `number` | — | `0.8` | Yes | minimum zoom scale. |
| `max_scale` | `number` | — | `2.5` | Yes | maximum zoom scale. |
| `pan_enabled` | `flag` | — | `true` | Yes | whether panning is enabled. |
| `scale_enabled` | `flag` | — | `true` | Yes | whether scaling is enabled. |
| `constrained` | `flag` | — | `true` | Yes | whether the child is constrained to the viewport. |
| `boundary_margin` | `edge` | — | `zero` | Yes | pan boundary outside the viewport. |
...
pan_enabled: true
boundary_margin: 20
_child: { _type: image, src: 'https://example.com/map.png' }
```
