...
Builds Flutter's [AspectRatio] for an `aspectRatio` node. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `aspect_ratio` | `number` | — | `1.0` | Yes | width-to-height ratio of the rendered box. |
