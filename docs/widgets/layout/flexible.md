...
Builds Flutter's [Flexible], supplying an empty child when required. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `flex` | `integer` | — | `1` | Yes | share of remaining main-axis space assigned to this child. |
| `fit` | `flexFit` | tight, loose | `loose` | Yes | content fitting mode. Values: tight \| loose. |
