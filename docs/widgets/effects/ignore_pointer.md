...
Builds Flutter's [IgnorePointer] to pass pointer events through. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `ignoring` | `flag` | — | `true` | Yes | when true, the subtree is invisible to hit-testing so pointer events pass through to widgets below. |
