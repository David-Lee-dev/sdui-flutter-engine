...
Builds Flutter's [AbsorbPointer] to absorb pointer events. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `absorbing` | `flag` | — | `true` | Yes | when true, the subtree looks normal but absorbs pointer events so neither it nor anything below receives them. |
