...
Builds Flutter's [Visibility] with independently resolved preservation flags. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `maintain_state` | `flag` | — | `false` | Yes | keeps the hidden child subtree mounted. |
| `maintain_size` | `flag` | — | `false` | Yes | keeps the hidden child’s layout space. |
| `visible` | `flag` | — | `true` | Yes | shows the child when true. |
