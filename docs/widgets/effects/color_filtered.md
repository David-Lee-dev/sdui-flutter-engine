...
Tints its child's painted output with a color blend. This primitive mirrors the corresponding Flutter widget described by its implementation.

## Properties

| Name | Type | Allowed values | Default | Bindable (`${}`) | Description |
| --- | --- | --- | --- | --- | --- |
| `color` | `color` | — | `null` | Yes | tint color; null returns the child unchanged. |
| `blend_mode` | `blendMode` | src_over, src_atop, src_in, dst_in, modulate, multiply, screen, overlay, darken, lighten, color, hue, saturation, luminosity, difference, exclusion, plus, clear | `src_over` | Yes | color compositing mode. |
...
color: '#336699'
blend_mode: src_over
_child: { _type: image, src: 'https://example.com/image.png' }
```
